#!/bin/sh
# build.sh — R11 (stage0-gcc-10.4.0) ★ THE PIVOT ★ driver.  RETARGETED 2026-07-01 (was 10.5.0);
# CORRECTED 2026-07-02 against the proven R9 (stage0-gcc-4.7.4) recipe.  gcc-10.4.0 is a seed-rooted
# MODERN C/C++ compiler — the artifact that closes B3's lower spine.
#
# ✅  WHY 10.4.0 (NOT 10.5.0): gcc-10.5 back-ported a C++11 HOST-compiler bootstrap requirement.  R9
#   gcc-4.7.4's C++11 is experimental/incomplete (no inheriting constructors / full thread_local) → it
#   CANNOT compile gcc-10.5's C++ source, and --disable-bootstrap makes it worse (host g++ compiles every
#   .cc directly, no stage2 rescue).  gcc-10.4.0 is the LAST C++98-bootstrappable gcc → R9's g++ builds it
#   DIRECTLY (zero new gcc rung).  build.ncl's CC import is the single swap-point if an intermediate is
#   ever preferred.
#
# ── KEY DELTAS vs R9 (why this is NOT a verbatim R9 clone) ──────────────────────────────────────────
#   1. BUILD-WITH-CXX: R9 used --disable-build-with-cxx (its host, gcc-4.0.4, is C-only).  gcc-10 is
#      C++-implemented and DROPPED C-only builds (gcc-4.8+) → we build WITH C++: CC=gcc-cc (R9's gcc),
#      CXX=gcc-cxx (R9's g++, linking R9's libstdc++.a).  No --disable-build-with-cxx (obsolete anyway).
#   2. gcc-cxx WRAPPER: net-new C++ host wrapper over R9's g++.  Uses the include_next-SAFE posture
#      (-nostdinc -nostdinc++ + EXPLICIT C++ dirs before musl) — see the wrapper block for the ordering
#      proof.  The naive "-nostdinc only, keep built-in C++ dirs" posture BREAKS <cstdlib>'s include_next.
#   3. libstdc++ os/generic ctype sed: R9 fix (d) TRANSFERS (load-bearing) — we still use the -gnu triplet
#      on musl, so gcc-10's musl awareness (keyed on *-linux-musl) does NOT rescue us.
#   4. CFLAGS_FOR_TARGET/CXXFLAGS_FOR_TARGET="-B -L $SR/lib -static": R9 fix (c) TRANSFERS (load-bearing) —
#      the libstdc++ configure LINK test needs -static + musl crt or it trips GCC_NO_EXECUTABLES.
#   5. --disable-libmudflap DROPPED (removed in gcc-4.9); --disable-libitm ADDED (still present, musl-risky).
#   6. .tar.xz (needs xz in the closure), not R9's .tar.bz2.
set -ex
VERSION="${MINIMAL_ARG_VERSION:-10.4.0}"   # 10.4.0 (NOT 10.5.0): last C++98-bootstrappable gcc — see banner
...
set -e
if [ "${OUT}" = "10" ]; then
  echo "R11-CXX-GATE: PASS (g++ compiled+ran C++14 + static libstdc++; got '${OUT}')" >&2
else
  echo "R11-CXX-GATE: FAIL (rc=${grc} got '${OUT}', want '10'); tail:" >&2; tail -12 "${GATE}/err" >&2 || true
  exit 1
fi
