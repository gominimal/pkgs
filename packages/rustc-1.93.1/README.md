# packages/rustc-1.93.1

Builds rustc 1.93.1 (with cargo and std) from the upstream `rustc-1.93.1-src.tar.xz`, using the
`rustc-1.92.0` package as x.py's stage0, with no network access, and installs it to the private prefix
`/usr/lib/rustc-1.93.1` (`bin/`, `lib/` and `BUILDINFO` only). `rustc-1.94.1` consumes that prefix as its
stage0 the same way.

## Stage0 wiring

`build.sh` generates `bootstrap.toml` with `[build] rustc`/`cargo` pointing at
`/usr/lib/rustc-1.92.0/bin/{rustc,cargo}`, so x.py uses those binaries instead of downloading the
`src/stage0` snapshot. Before building it asserts the stage0 runs (`--version` contains 1.92.0,
`--print sysroot` equals the prefix) and that the extracted source is the right one
(`src/version` is 1.93.1 and `src/stage0` pins the stage0's version).
...
1. `rustc --version` contains 1.93.1.
...
| file | role |
|---|---|
| `build.ncl` | spec: stage0 build_dep, source, system LLVM, one `OutputData` glob over `usr/lib/rustc-1.93.1/**`; empty on arm64 |
...
| `rustc-1.93.1.answers` | sha256 pins for `bin/rustc` and `bin/cargo`; none yet, so the seal check is skipped |
