...
for t in gcc g++ ld ar ranlib make sed grep tar find xargs sha256sum patch; do
...
for x in /usr/lib/ecj-bootstrap-3.2.2/bin/javac /usr/lib/classpath-devel/share/classpath/glibj.zip; do [ -e "$x" ] || { echo "@NAME@: missing boot artifact $x" >&2; exit 1; }; done
...
cat > "${CCDIR}/gcc" <<WRAP
#!/bin/sh
for a in "\$@"; do case "\$a" in -c|-S|-E|-M|-MM) exec "${BGCC}" ${INC} -fcommon -Wno-error=implicit-function-declaration -Wno-error=incompatible-pointer-types -Wno-error=int-conversion "\$@" ;; esac; done
exec "${BGCC}" ${INC} -fcommon -Wno-error=implicit-function-declaration -Wno-error=incompatible-pointer-types -Wno-error=int-conversion "\$@" ${LNK}
WRAP
cat > "${CCDIR}/g++" <<WRAP
#!/bin/sh
