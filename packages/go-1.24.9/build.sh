#!/bin/sh
# go-1.24.9: one rung of the Go bootstrap ladder.  Builds Go 1.24.9 from go1.24.9.src.tar.gz with the
# previous rung (/usr/lib/go-1.22.6) as GOROOT_BOOTSTRAP and installs the whole GOROOT to
# /usr/lib/go-1.24.9 for the next rung to consume.
...
mkdir -p "$OUTPUT_DIR"
VERSION="${MINIMAL_ARG_VERSION:-1.24.9}"
SRC_TARBALL="go${VERSION}.src.tar.gz"
SRC_SHA=c72f81ba54fe00efe7f3e7499d400979246881b13b775e9a9bb85541c11be695
BUILDROOT="$(pwd)"
PREFIX_REL="usr/lib/go-${VERSION}"
BOOTSTRAP_PREFIX=/usr/lib/go-1.22.6   # the previous rung (a build_dep)
GCC_VERSION=15.2.0
...
[ -n "${BGCC}" ] || { echo "go-1.24.9: gcc not on PATH" >&2; exit 1; }
for t in as ld ar ranlib make sed grep tar bash uname sha256sum cp mkdir; do
  command -v "$t" >/dev/null 2>&1 || { echo "go-1.24.9 infra: '$t' not on PATH" >&2; exit 1; }
done
GCCVER="$("${BGCC}" -dumpversion 2>/dev/null || echo unknown)"
[ "${GCCVER}" = "${GCC_VERSION}" ] || {
  echo "go-1.24.9: gcc -dumpversion='${GCCVER}', expected '${GCC_VERSION}' (gcc-15.2.0-glibc)" >&2; exit 1; }
[ -x "${BOOTSTRAP_PREFIX}/bin/go" ] || {
  echo "go-1.24.9 infra: predecessor go-1.22.6 is missing at ${BOOTSTRAP_PREFIX}/bin/go — that rung IS the anchor" >&2
  exit 1; }
[ -e "${SR}/lib/libc.so" ] || { echo "go-1.24.9: glibc sysroot missing at ${SR}" >&2; exit 1; }
[ -f "${SRC_TARBALL}" ]    || { echo "go-1.24.9 infra: source ${SRC_TARBALL} absent" >&2; exit 1; }
echo "${SRC_SHA}  ${SRC_TARBALL}" | sha256sum -c - || {
  echo "go-1.24.9 infra: source sha mismatch — refusing to build unpinned" >&2; exit 1; }
...
[ -x "${BOOT}/bin/go" ] || { echo "go-1.24.9 infra: writable go-1.22.6 copy has no bin/go" >&2; exit 1; }
...
echo "go-1.24.9: bootstrap Go = ${BOOTVER}" >&2
case "${BOOTVER}" in *go1.22.6*) : ;; *)
  echo "go-1.24.9 infra: bootstrap go version is '${BOOTVER}', expected go1.22.6 (the predecessor rung)" >&2
  exit 1 ;; esac
...
GODIR=go
[ -d "${GODIR}/src" ] || { echo "go-1.24.9 infra: expected ./go/src after untar. cwd:" >&2; ls -la >&2; exit 1; }
...
[ "${NG}" -gt 0 ] || { echo "go-1.24.9 infra: src/cmd/dist has no .go files (${NG}) — unexpected source" >&2; exit 1; }
echo "go-1.24.9: self-hosted dist confirmed (${NG} .go files) — requires the C-rooted go-1.4" >&2
...
[ -x "${DEST}/bin/go" ] || { echo "go-1.24.9: FATAL bin/go missing after install" >&2; exit 1; }
...
echo "go-1.24.9: produced toolchain = ${VOUT}" >&2
case "${VOUT}" in *"go${VERSION}"*) : ;; *)
  echo "go-1.24.9: FATAL produced toolchain reports '${VOUT}', expected go${VERSION}" >&2; exit 1 ;; esac
set +e
...
if [ "${GOUT}" = "GO-1249-GATE:21" ]; then
  ...
  echo "GO1249-GATE: FAIL (rc=${grc}, got '${GOUT}', want 'GO-1249-GATE:21'); tail:" >&2
...
mkdir -p "${OUTPUT_DIR}/usr/share/go-${VERSION}"
{
  ...
  echo "gate:         compiled+ran a stdlib program, asserted GO-1249-GATE:21 + version go${VERSION}"
