# packages/rustc-1.96.0

Builds rustc 1.96.0 (with cargo and std) from the upstream `rustc-1.96.0-src.tar.xz`, using the
`rustc-1.95.0` package as x.py's stage0, with no network access, and installs it to the private prefix
`/usr/lib/rustc-1.96.0` (`bin/`, `lib/` and `BUILDINFO` only). `packages/rust` consumes that prefix as its
stage0 the same way.

## Stage0 wiring

`build.sh` generates `bootstrap.toml` with `[build] rustc`/`cargo` pointing at
`/usr/lib/rustc-1.95.0/bin/{rustc,cargo}`, so x.py uses those binaries instead of downloading the
`src/stage0` snapshot. Before building it asserts the stage0 runs (`--version` contains 1.95.0,
`--print sysroot` equals the prefix) and that the extracted source is the right one
(`src/version` is 1.96.0 and `src/stage0` pins the stage0's version).
...
1. `rustc --version` contains 1.96.0.
...
| file | role |
|---|---|
| `build.ncl` | spec: stage0 build_dep, source, system LLVM, one `OutputData` glob over `usr/lib/rustc-1.96.0/**`; empty on arm64 |
...
| `rustc-1.96.0.answers` | sha256 pins for `bin/rustc` and `bin/cargo`; none yet, so the seal check is skipped |
...
This rung's newer bootstrap needs four more things to stay offline, all in `build.sh`:
- it bootstraps from a copy of the stage0 with any bundled `rust-src` removed, since x.py would
  otherwise compile pre-rename `proc_macro::tracked_env` code against it;
- `bootstrap.toml` sets `build.vendor = false`, `dist.vendor = false`, `dist.src-tarball = false`,
  `rust.remap-debuginfo = false` and an explicit `tools` list without `src`;
- bootstrap's `vendor.rs` is patched to skip the generate-copyright vendoring when
  `BOOTSTRAP_SKIP_VENDOR` is set (build tooling only, nothing shipped);
- `x.py install` names its components instead of installing everything.
