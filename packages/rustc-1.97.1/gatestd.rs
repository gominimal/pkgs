...
// The pass value 42 is never written in the success path: it is the sum of ten independently
// computed quantities, so a compiler that miscompiled `!=`/`==` into a constant cannot produce it.
// Each check also exits with its own code (111-120) so a failure names the miscompiled construct.
...
fn main() {
    ...
    for h in handles {
        ...
        // on the guard executing, which defeats the non-vacuity argument.
        let joined = h.join();
        if joined.is_err() {
