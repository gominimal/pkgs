// gate190.rs: GATE-3 for the mrustc-built rustc 1.90.0. Ten std constructs are compiled,
// linked and executed. The pass value 42 is only ever the sum of ten computed quantities, so a
// miscompiled comparison cannot produce it; each check also exits with its own code (111-120)
// so a failure names the construct.
//
...
fn main() {
    ...
    // Proves the archive was read back, not just written.
    ...
    for h in handles {
        // join() is called unconditionally and its result bound before being tested: a
        // side-effecting call inside the guard condition would make the accumulated value
        // depend on the guard executing.
        let joined = h.join();
        if joined.is_err() {
