#!/bin/sh
# ============================================================================================
# build.sh — go-1.17.13: the FIRST self-hosted rung of the Go ladder (issue #19).
#
#   IS:     /usr/lib/go-1.17.13 — a Go 1.17.13 toolchain built BY go-1.4 (the C-rooted joint).
#           go1.17's own src/cmd/dist is 15 .go files (self-hosted), so it REQUIRES a working Go
...
#           go-1.17.13 -> go-1.20.14 -> go-1.22.6 -> go-1.24.x -> go.
#
...
mkdir -p "$OUTPUT_DIR"
VERSION="${MINIMAL_ARG_VERSION:-1.17.13}"
SRC_TARBALL="go${VERSION}.src.tar.gz"
SRC_SHA=a1a48b23afb206f95e7bbaa9b898d965f90826f6f1d1fc0c1d784ada0cd300fd
BUILDROOT="$(pwd)"
PREFIX_REL="usr/lib/go-${VERSION}"
BOOTSTRAP_PREFIX=/usr/lib/go-1.4   # the C-rooted predecessor (a build_dep of this rung)
GCC_VERSION=15.2.0
...
[ -n "${BGCC}" ] || { echo "go-1.17.13 infra: B5 gcc not on PATH" >&2; exit 1; }
for t in as ld ar ranlib make sed grep tar bash uname sha256sum cp mkdir; do
  command -v "$t" >/dev/null 2>&1 || { echo "go-1.17.13 infra: '$t' not on PATH" >&2; exit 1; }
done
GCCVER="$("${BGCC}" -dumpversion 2>/dev/null || echo unknown)"
[ "${GCCVER}" = "${GCC_VERSION}" ] || {
  echo "go-1.17.13 infra: gcc -dumpversion='${GCCVER}', expected '${GCC_VERSION}' (B5)" >&2; exit 1; }
[ -x "${BOOTSTRAP_PREFIX}/bin/go" ] || {
  echo "go-1.17.13 infra: the C-rooted go-1.4 is missing at ${BOOTSTRAP_PREFIX}/bin/go — that rung IS the anchor" >&2
  exit 1; }
[ -e "${SR}/lib/libc.so" ] || { echo "go-1.17.13 infra: B4 sysroot missing at ${SR}" >&2; exit 1; }
[ -f "${SRC_TARBALL}" ]    || { echo "go-1.17.13 infra: source ${SRC_TARBALL} absent" >&2; exit 1; }
echo "${SRC_SHA}  ${SRC_TARBALL}" | sha256sum -c - || {
  echo "go-1.17.13 infra: source sha mismatch — refusing to build unpinned" >&2; exit 1; }
...
# ============================================================================================
BOOT="${BUILDROOT}/go14-boot"
cp -a "${BOOTSTRAP_PREFIX}" "${BOOT}"
[ -x "${BOOT}/bin/go" ] || { echo "go-1.17.13 infra: writable go-1.4 copy has no bin/go" >&2; exit 1; }
...
echo "go-1.17.13: bootstrap Go = ${BOOTVER}" >&2
case "${BOOTVER}" in *go1.4*) : ;; *)
  echo "go-1.17.13 infra: bootstrap go version is '${BOOTVER}', expected go1.4.x (the C-rooted rung)" >&2
  exit 1 ;; esac
...
GODIR=go
[ -d "${GODIR}/src" ] || { echo "go-1.17.13 infra: expected ./go/src after untar. cwd:" >&2; ls -la >&2; exit 1; }
...
[ "${NG}" -gt 0 ] || { echo "go-1.17.13 infra: src/cmd/dist has no .go files (${NG}) — unexpected source" >&2; exit 1; }
echo "go-1.17.13: self-hosted dist confirmed (${NG} .go files) — requires the C-rooted go-1.4" >&2
...
[ -x "${DEST}/bin/go" ] || { echo "go-1.17.13: FATAL bin/go missing after install" >&2; exit 1; }
...
echo "go-1.17.13: produced toolchain = ${VOUT}" >&2
case "${VOUT}" in *"go${VERSION}"*) : ;; *)
  echo "go-1.17.13: FATAL produced toolchain reports '${VOUT}', expected go${VERSION}" >&2; exit 1 ;; esac
set +e
GOUT="$(cd "${GATE}" && GOROOT="${DEST}" GOPATH="${GATE}/gp" GOCACHE="${GATE}/gc" CGO_ENABLED=0 \
  TMPDIR="${GOTMP}" timeout 300 "${DEST}/bin/go" run hello.go 2>"${GATE}/err")"
grc=$?
set -e
if [ "${GOUT}" = "GO-1713-GATE:21" ]; then
  echo "GO1713-GATE: PASS (go-1.4-rooted Go 1.17.13 compiled AND RAN a stdlib program; got '${GOUT}')" >&2
else
  echo "GO1713-GATE: FAIL (rc=${grc}, got '${GOUT}', want 'GO-1713-GATE:21'); tail:" >&2
