#!/bin/sh
# go-1.17.13: one rung of the Go bootstrap ladder.  Builds Go 1.17.13 from go1.17.13.src.tar.gz with the
# previous rung (/usr/lib/go-1.4) as GOROOT_BOOTSTRAP and installs the whole GOROOT to
# /usr/lib/go-1.17.13 for the next rung to consume.
...
BOOTSTRAP_PREFIX=/usr/lib/go-1.4   # the previous rung (a build_dep)
GCC_VERSION=15.2.0
...
[ -n "${BGCC}" ] || { echo "go-1.17.13: gcc not on PATH" >&2; exit 1; }
...
[ "${GCCVER}" = "${GCC_VERSION}" ] || {
  echo "go-1.17.13: gcc -dumpversion='${GCCVER}', expected '${GCC_VERSION}' (gcc-15.2.0-glibc)" >&2; exit 1; }
[ -x "${BOOTSTRAP_PREFIX}/bin/go" ] || {
  ...
  exit 1; }
[ -e "${SR}/lib/libc.so" ] || { echo "go-1.17.13: glibc sysroot missing at ${SR}" >&2; exit 1; }
...
mkdir -p "${OUTPUT_DIR}/usr/share/go-${VERSION}"
{
  ...
  echo "bootstrapped: ${BOOTVER}  (from ${BOOTSTRAP_PREFIX} — the C-rooted go-1.4)"
