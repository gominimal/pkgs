#!/bin/sh
# Imported from npm `yarn` (1.22.22, node) by pkgmgr import npm.
set -eu
...
prefix="$OUTPUT_DIR/usr/libexec/yarn"
mkdir -p "$prefix"
...
for _bin in node_modules/.bin/*; do
  [ -e "$_bin" ] || continue
  _tool=${_bin##*/}
  ln -s "../libexec/yarn/node_modules/.bin/$_tool" "$OUTPUT_DIR/usr/bin/$_tool"
done
