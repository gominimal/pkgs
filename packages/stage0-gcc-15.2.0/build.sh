#!/bin/sh
# stage0-gcc-15.2.0: builds gcc 15.2.0 (C and C++: cc1, cc1plus, g++, static libstdc++) from
# gcc-15.2.0.tar.xz with stage0-gcc-10.4.0's gcc/g++ as CC/CXX, linked statically
...
# gcc-15 requires a C++14 host compiler; gcc-10.4.0 provides it, so no intermediate gcc is needed.
...
[ -n "${BUILDER_GCC}" ] || { echo "gcc-15.2.0: builder gcc not on PATH" >&2; exit 1; }
[ -n "${BUILDER_GXX}" ] || { echo "gcc-15.2.0: builder g++ not on PATH (need a C++ HOST compiler)" >&2; exit 1; }
command -v as >/dev/null 2>&1 || { echo "gcc-15.2.0: as (binutils) not on PATH" >&2; exit 1; }
[ -f "${GCC_MATH}/lib/libgmp.a" ] || { echo "gcc-15.2.0: gmp/mpfr/mpc missing at ${GCC_MATH}" >&2; exit 1; }
[ -f "${SR}/lib/libc.a" ] || { echo "gcc-15.2.0: musl-1.2.5 sysroot missing at ${SR}" >&2; exit 1; }
# The host g++ links the host libstdc++.a and needs its C++ headers; fail here rather
# than deep in the build.
[ -f /usr/lib/libstdc++.a ] || { echo "gcc-15.2.0: builder libstdc++.a missing at /usr/lib (the builder gcc must ship it as cxx_libs)" >&2; exit 1; }
...
[ -d "${GI}" ] || { echo "gcc-15.2.0: builder-gcc freestanding include dir not found ('${GI}')" >&2; exit 1; }
...
[ -n "${CXXCFG}" ] || { echo "gcc-15.2.0: builder libstdc++ target headers (c++config.h) not found under /usr/include/c++/*/${TARGET}/bits" >&2; exit 1; }
...
for t in bison yacc flex lex m4 gperf perl autoconf autoheader autom4te aclocal automake autoreconf libtoolize; do
  printf '#!/bin/sh\necho "gcc-15.2.0 MODEL-B GUARD: %s invoked ($*) -> a generated file is being regenerated; add it to the touch list." >&2\nexit 1\n' "$t" > "${STUBS}/${t}"
  chmod +x "${STUBS}/${t}"
done
...
#   --with-native-system-header-dir=$SR/include: gcc-15's libstdc++ probes glibc-only headers
#     (sys/single_threaded.h, execinfo.h) with __has_include and has no configure knob for it.
#     Without this the built xgcc searches /usr/include, finds glibc's sys/single_threaded.h, and
#     libsupc++ calls glibc-only __libc_single_threaded. With the musl sysroot as the system header
#     dir those headers are absent and libstdc++ falls back to __gthread_active_p().
