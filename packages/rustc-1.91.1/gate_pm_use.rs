...
//
// Edition 2015 (no --edition is passed), so `#[macro_use] extern crate` is how the derive comes
// into scope.
...
fn main() {
    let t = Target;
    // val() exists ONLY because the proc macro ran. If expansion silently produced nothing, this
    // file does not compile — which GATE-4 reports as a distinct failure.
    let v = t.val();
    if v != 13 {
        // The macro expanded, but produced the wrong value: a codegen/expansion miscompile rather
        // than a plumbing failure. Named separately so the log says which.
