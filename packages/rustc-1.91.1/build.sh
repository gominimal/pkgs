...
# src/stage0 must pin exactly our stage0 version.
grep -q "compiler_version=${STAGE0_VERSION}" "${BUILDROOT}/src/stage0" || {
  echo "rustc: FATAL src/stage0 does not pin compiler_version=${STAGE0_VERSION} — the bootstrap pairing is wrong" >&2
  grep -i 'compiler_' "${BUILDROOT}/src/stage0" >&2 || true
  exit 1; }
echo "rustc SOURCE: OK — src/version=${SVER}, src/stage0 pins ${STAGE0_VERSION} (pairing confirmed)" >&2
