// gatestd.rs — GATE-3.  THE functional gate for this rung's rustc.
//
// AUDIT COPY.  build.sh embeds this body inline (a heredoc); this file is the human-readable
// source of truth.  Version-agnostic edition-2015 Rust — identical across every rung; ported
// verbatim from packages/mrustc-rustc-1.90.0/gate190.rs.
//
// ── WHY THIS SHAPE ───────────────────────────────────────────────────────────────────────────
// Upstream's own checks cannot do this job: a compile-only sample returns a LITERAL 0 (vacuous
// even if executed), and hello_world returns () so its exit status is 0 unless the process dies
// ("does not crash").  A subtly-wrong rustc reliably still compiles println! while getting vtable
// layout, unwind tables, or TLS/atomics wrong — exactly what hello.rs cannot reach.
//
// ── NON-VACUITY ──────────────────────────────────────────────────────────────────────────────
// The pass value 42 is NEVER written in the success path.  It is only ever the SUM of ten
// independently computed quantities.  A compiler that miscompiled `!=`/`==` into a constant cannot
// manufacture it — it would have to compute ten specific wrong values that happen to sum to 42.
// Each check ALSO exits with its own code (111-120) so a red gate NAMES the miscompiled construct.
//
...
fn main() {
    ...
    for h in handles {
        ...
        // on the guard executing, which defeats the whole non-vacuity argument.  (Measured — the
