#!/bin/bash
# openjdk-12: one rung of the JVM ladder (jikes-1.22 -> classpath-0.93 -> jamvm-1.5.1 -> ant-bootstrap-1.8.4 -> ecj-bootstrap-3.2.2 -> classpath-0.99 -> classpath-devel -> jamvm-2.0.0 -> ecj4-bootstrap-4.2.1 -> icedtea-7 -> icedtea-8 -> openjdk-9 -> ... -> openjdk-25).
# OpenJDK 12+33 from the GitHub tag archive, built with ../openjdk-11 as the boot JDK.
set -eu
trap 'echo "openjdk-12: failed at line $LINENO: $BASH_COMMAND" >&2' ERR
...
[ "${GCCVER}" = "${GCC_VERSION}" ] || { echo "openjdk-12: gcc -dumpversion='${GCCVER}', expected '${GCC_VERSION}'" >&2; exit 1; }
[ -e "${SR}/lib/libc.so" ] || { echo "openjdk-12: glibc sysroot missing at ${SR}" >&2; exit 1; }
[ -e "${LOADER}" ] || { echo "openjdk-12: glibc loader missing at ${LOADER}" >&2; exit 1; }
for x in /usr/lib/openjdk-11/bin/javac /usr/lib/openjdk-11/bin/java /usr/include/gif_lib.h /usr/include/freetype2/freetype/freetype.h /usr/include/X11/Intrinsic.h; do [ -e "$x" ] || { echo "openjdk-12: missing boot artifact $x" >&2; exit 1; }; done
...
if grep -q '/build/output' "${FIXLIB}/libc.so"; then echo "openjdk-12: libc.so fixup failed" >&2; exit 1; fi
...
[ -n "$(echo "${CXXINC}" | grep -o 'c++')" ] || { echo "openjdk-12: cannot find g++'s libstdc++ include dirs" >&2; exit 1; }
...
BOOT=/usr/lib/openjdk-11
# --- P2 unpack (the GitHub tag archive; shipped binaries and jars removed, as Guix does) ---
mkdir src && tar -xzf jdk-12+33.tar.gz -C src --strip-components=1
cd src
...
bash ./configure --with-boot-jdk="${BOOT}" --disable-option-checking --disable-warnings-as-errors --with-native-debug-symbols=none \
  ...
  || { grep -n -iE 'error|could not|cannot|not found' ../configure.log | tail -8 >&2 || true; echo "openjdk-12: configure failed" >&2; exit 1; }
make JOBS="${JOBS}" all > ../make.log 2>&1 \
  || { grep -nE 'error:|Error [0-9]|\*\*\* \[' ../make.log | grep -v Werror | tail -8 >&2 || true; tail -10 ../make.log >&2; echo "openjdk-12: make failed" >&2; exit 1; }
IMG="$(ls -d build/*/images/jdk | head -1)"
[ -x "${IMG}/bin/java" ] || { echo "openjdk-12: no jdk image" >&2; exit 1; }
...
[ "$OUT" = "OJ:21:12:abab" ] || { echo "openjdk-12: gate printed '$OUT'" >&2; exit 1; }
