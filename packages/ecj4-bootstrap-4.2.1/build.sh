#!/bin/bash
# ecj4-bootstrap-4.2.1: one rung of the JVM ladder (jikes-1.22 -> classpath-0.93 -> jamvm-1.5.1 -> ant-bootstrap-1.8.4 -> ecj-bootstrap-3.2.2 -> classpath-0.99 -> classpath-devel -> jamvm-2.0.0 -> ecj4-bootstrap-4.2.1 -> icedtea-7 -> icedtea-8 -> openjdk-9 -> ... -> openjdk-25).
...
for x in /usr/lib/jamvm-2.0.0/bin/jamvm /usr/lib/ecj-bootstrap-3.2.2/share/java/ecj-bootstrap.jar /usr/lib/classpath-devel/share/classpath/glibj.zip /usr/lib/ant-bootstrap-1.8.4/lib/ant.jar; do [ -e "$x" ] || { echo "ecj4-bootstrap-4.2.1: missing boot artifact $x" >&2; exit 1; }; done
...
[ -n "$(echo "${CXXINC}" | grep -o 'c++')" ] || { echo "ecj4-bootstrap-4.2.1: cannot find g++'s libstdc++ include dirs" >&2; exit 1; }
