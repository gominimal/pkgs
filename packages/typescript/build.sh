#!/bin/sh
# Imported from npm `typescript` (7.0.2, node) by pkgmgr import npm.
set -eu
...
prefix="$OUTPUT_DIR/usr/libexec/typescript"
mkdir -p "$prefix"
...
for _bin in node_modules/.bin/*; do
  [ -e "$_bin" ] || continue
  _tool=${_bin##*/}
  ln -s "../libexec/typescript/node_modules/.bin/$_tool" "$OUTPUT_DIR/usr/bin/$_tool"
done
