...
for t in gcc g++ ld ar ranlib make sed grep tar find xargs sha256sum python3 awk diff fastjar zip autoreconf; do
...
for x in /usr/lib/ecj-bootstrap-3.2.2/bin/javac /usr/lib/jamvm-1.5.1/bin/jamvm /usr/lib/classpath-0.99/bin/javah; do [ -e "$x" ] || { echo "classpath-devel: missing boot artifact $x" >&2; exit 1; }; done
...
./configure --prefix="${PREFIX}" --with-ecj-jar="${ECJJAR}" --with-javac="${JAVAC}" JAVA="${JAMVM}" GCJ_JAVAC_TRUE=no ac_cv_prog_java_works=yes \
  ...
  || { tail -12 ../configure.log >&2; grep -n -B3 -A25 'javac.*works' config.log | tail -60 >&2 || true
       printf 'public class T { public static void main(String[] a){ System.out.println("ok"); } }\n' > /tmp/T.java; (cd /tmp && javac -Xlint:unchecked T.java; echo "manual javac rc=$?"; ls -la /tmp/T.class) >&2 2>&1 || true
       echo "classpath-devel: configure failed" >&2; exit 1; }
