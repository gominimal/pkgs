#!/bin/bash
# openjdk-11: one rung of the JVM ladder (jikes-1.22 -> classpath-0.93 -> jamvm-1.5.1 -> ant-bootstrap-1.8.4 -> ecj-bootstrap-3.2.2 -> classpath-0.99 -> classpath-devel -> jamvm-2.0.0 -> ecj4-bootstrap-4.2.1 -> icedtea-7 -> icedtea-8 -> openjdk-9 -> ... -> openjdk-25).
# OpenJDK 11.0.22 from the GitHub tag archive, built with ../openjdk-10 as the boot JDK (the JDK 10 boot needs -XX:UseAVX=2).
set -eu
trap 'echo "openjdk-11: failed at line $LINENO: $BASH_COMMAND" >&2' ERR
...
# --- P0 preconditions ---
[ "$(uname -m)" = x86_64 ] || { echo "openjdk-11: amd64 ladder rung on $(uname -m)" >&2; exit 1; }
for t in gcc g++ ld ar ranlib make sed grep tar find xargs sha256sum python3 awk diff xz gawk patch zip unzip cpio file which pkg-config objcopy readelf autoconf; do
  command -v "$t" >/dev/null 2>&1 || { echo "openjdk-11: '$t' not on PATH" >&2; exit 1; }
done
...
if grep -q '/build/output' "${FIXLIB}/libc.so"; then echo "openjdk-11: libc.so fixup failed" >&2; exit 1; fi
...
[ -n "$(echo "${CXXINC}" | grep -o 'c++')" ] || { echo "openjdk-11: cannot find g++'s libstdc++ include dirs" >&2; exit 1; }
...
BOOT=/usr/lib/openjdk-10
# --- P2 unpack (the GitHub tag archive; shipped binaries and jars removed, as Guix does) ---
mkdir src && tar -xJf openjdk-11.0.22-ga.tar.xz -C src --strip-components=1
cd src
...
patch -p1 < ../openjdk-11-classlist-reproducibility.patch
...
[ -x "${IMG}/bin/java" ] || { echo "openjdk-11: no jdk image" >&2; exit 1; }
...
[ "$OUT" = "OJ:21:11:abab" ] || { echo "openjdk-11: gate printed '$OUT'" >&2; exit 1; }
