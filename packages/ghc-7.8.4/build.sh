#!/bin/bash
# ghc-7.8.4: one rung of the GHC ladder (7.6.3 -> 7.8.4 -> 7.10.3 -> 8.2.2 -> 8.6.5 -> 8.10.7 -> 9.2.8 -> 9.6.7 -> ghc).
# Builds GHC 7.8.4 from the pinned source tarball with the previous rung (/usr/lib/ghc-7.6.3) as the
# boot compiler and installs the whole compiler to /usr/lib/ghc-7.8.4 for the next rung.
...
SRC_SHA=c319cd94adb284177ed0e6d21546ed0b900ad84b86b87c06a99eac35152982c4
BUILDROOT="$(pwd)"
...
# Pre-8.0 C on a modern gcc: gnu99, no PIE (static objects are linked into PIE-unaware binaries),
