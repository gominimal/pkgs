#!/bin/bash
# icedtea-7: one rung of the JVM ladder (jikes-1.22 -> classpath-0.93 -> jamvm-1.5.1 -> ant-bootstrap-1.8.4 -> ecj-bootstrap-3.2.2 -> classpath-0.99 -> classpath-devel -> jamvm-2.0.0 -> ecj4-bootstrap-4.2.1 -> icedtea-7 -> icedtea-8 -> openjdk-9 .. 25).
...
for t in gcc g++ ld ar ranlib make sed grep tar find xargs sha256sum gawk xz bzip2 gzip patch zip unzip cpio file which pkg-config objcopy; do
