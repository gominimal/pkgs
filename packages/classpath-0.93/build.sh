#!/bin/bash
# classpath-0.93: one rung of the JVM ladder (jikes-1.22 -> classpath-0.93 -> jamvm-1.5.1 -> ant-bootstrap-1.8.4 -> ecj-bootstrap-3.2.2 -> classpath-0.99 -> classpath-devel -> jamvm-2.0.0 -> ecj4-bootstrap-4.2.1 -> icedtea-7 -> icedtea-8 -> openjdk-9 -> ... -> openjdk-25).
# GNU Classpath 0.93 (the class library), compiled by jikes; the C parts by the sysroot gcc.
set -eu
trap 'echo "classpath-0.93: failed at line $LINENO: $BASH_COMMAND" >&2' ERR
...
for t in gcc g++ ld ar ranlib make sed grep tar find xargs sha256sum python3 awk diff zip; do
...
[ "${GCCVER}" = "${GCC_VERSION}" ] || { echo "classpath-0.93: gcc -dumpversion='${GCCVER}', expected '${GCC_VERSION}'" >&2; exit 1; }
[ -e "${SR}/lib/libc.so" ] || { echo "classpath-0.93: glibc sysroot missing at ${SR}" >&2; exit 1; }
[ -e "${LOADER}" ] || { echo "classpath-0.93: glibc loader missing at ${LOADER}" >&2; exit 1; }
for x in /usr/lib/jikes-1.22/bin/jikes; do [ -e "$x" ] || { echo "classpath-0.93: missing boot artifact $x" >&2; exit 1; }; done
...
if grep -q '/build/output' "${FIXLIB}/libc.so"; then echo "classpath-0.93: libc.so fixup failed" >&2; exit 1; fi
...
[ -n "$(echo "${CXXINC}" | grep -o 'c++')" ] || { echo "classpath-0.93: cannot find g++'s libstdc++ include dirs" >&2; exit 1; }
...
JIKES=/usr/lib/jikes-1.22/bin/jikes
# --- P2 build ---
mkdir src && tar -xzf classpath-0.93.tar.gz -C src --strip-components=1
cd src
JAVAC="${JIKES}" ./configure --prefix="${PREFIX}" --with-jikes="${JIKES}" --disable-Werror --disable-gmp --disable-gtk-peer --disable-gconf-peer \
  --disable-plugin --disable-dssi --disable-alsa --disable-gjdoc --disable-examples --disable-tools > ../configure.log 2>&1 \
  || { tail -30 ../configure.log >&2; echo "classpath-0.93: configure failed" >&2; exit 1; }
make -j"${JOBS}" > ../make.log 2>&1 || { grep -n -m5 -iE ' error|Error [0-9]' ../make.log >&2 || true; tail -20 ../make.log >&2; echo "classpath-0.93: make failed" >&2; exit 1; }
make install DESTDIR="${OUTPUT_DIR}" > ../install.log 2>&1 || { tail -20 ../install.log >&2; echo "classpath-0.93: install failed" >&2; exit 1; }
cd "${BUILDROOT}"
# --- P3 gate ---
...
classpath-0.93
source: classpath-0.93.tar.gz
javac: /usr/lib/jikes-1.22/bin/jikes
c compiler: gcc ${GCC_VERSION}; sysroot: ${SR}
EOF
echo "classpath-0.93: installed to ${PREFIX}"
