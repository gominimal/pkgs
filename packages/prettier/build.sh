#!/bin/sh
# Imported from npm `prettier` (3.9.8, node) by pkgmgr import npm.
set -eu
...
prefix="$OUTPUT_DIR/usr/libexec/prettier"
mkdir -p "$prefix"
...
for _bin in node_modules/.bin/*; do
  [ -e "$_bin" ] || continue
  _tool=${_bin##*/}
  ln -s "../libexec/prettier/node_modules/.bin/$_tool" "$OUTPUT_DIR/usr/bin/$_tool"
done
