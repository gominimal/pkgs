#!/bin/sh
# ============================================================================================
# build.sh — go-1.22.6: the FIRST self-hosted rung of the Go ladder (issue #19).
#
#   IS:     /usr/lib/go-1.22.6 — a Go 1.17.13 toolchain built BY go-1.20.14.
#           go1.22.6 is self-hosted (dist in Go), so it REQUIRES a working Go
...
#           go-1.22.6 -> go-1.20.14 -> go-1.22.6 -> go-1.24.x -> go.
#
...
BOOTSTRAP_PREFIX=/usr/lib/go-1.20.14   # the predecessor rung (a build_dep of this rung)
GCC_VERSION=15.2.0
...
[ -n "${BGCC}" ] || { echo "go-1.22.6 infra: B5 gcc not on PATH" >&2; exit 1; }
...
[ "${GCCVER}" = "${GCC_VERSION}" ] || {
  echo "go-1.22.6 infra: gcc -dumpversion='${GCCVER}', expected '${GCC_VERSION}' (B5)" >&2; exit 1; }
[ -x "${BOOTSTRAP_PREFIX}/bin/go" ] || {
  ...
  exit 1; }
[ -e "${SR}/lib/libc.so" ] || { echo "go-1.22.6 infra: B4 sysroot missing at ${SR}" >&2; exit 1; }
...
if [ "${GOUT}" = "GO-1226-GATE:21" ]; then
  ...
  echo "GO1226-GATE: FAIL (rc=${grc}, got '${GOUT}', want 'GO-1713-GATE:21'); tail:" >&2
