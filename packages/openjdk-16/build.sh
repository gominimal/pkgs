#!/bin/bash
# openjdk-16: one rung of the JVM ladder (jikes-1.22 -> classpath-0.93 -> jamvm-1.5.1 -> ant-bootstrap-1.8.4 -> ecj-bootstrap-3.2.2 -> classpath-0.99 -> classpath-devel -> jamvm-2.0.0 -> ecj4-bootstrap-4.2.1 -> icedtea-7 -> icedtea-8 -> openjdk-9 -> ... -> openjdk-25).
# OpenJDK 16.0.2 from the GitHub tag archive, built with ../openjdk-15 as the boot JDK.
set -eu
trap 'echo "openjdk-16: failed at line $LINENO: $BASH_COMMAND" >&2' ERR
...
# --- P0 preconditions ---
[ "$(uname -m)" = x86_64 ] || { echo "openjdk-16: amd64 ladder rung on $(uname -m)" >&2; exit 1; }
for t in gcc g++ ld ar ranlib make sed grep tar find xargs sha256sum python3 awk diff gzip gawk patch zip unzip cpio file which pkg-config objcopy readelf autoconf; do
  command -v "$t" >/dev/null 2>&1 || { echo "openjdk-16: '$t' not on PATH" >&2; exit 1; }
done
BGCC="$(command -v gcc)"; BGXX="$(command -v g++)"
GCCVER="$("${BGCC}" -dumpversion 2>/dev/null || echo unknown)"
[ "${GCCVER}" = "${GCC_VERSION}" ] || { echo "openjdk-16: gcc -dumpversion='${GCCVER}', expected '${GCC_VERSION}'" >&2; exit 1; }
[ -e "${SR}/lib/libc.so" ] || { echo "openjdk-16: glibc sysroot missing at ${SR}" >&2; exit 1; }
[ -e "${LOADER}" ] || { echo "openjdk-16: glibc loader missing at ${LOADER}" >&2; exit 1; }
for x in /usr/lib/openjdk-15/bin/javac /usr/lib/openjdk-15/bin/java /usr/include/gif_lib.h /usr/include/freetype2/freetype/freetype.h /usr/include/X11/Intrinsic.h; do [ -e "$x" ] || { echo "openjdk-16: missing boot artifact $x" >&2; exit 1; }; done
...
if grep -q '/build/output' "${FIXLIB}/libc.so"; then echo "openjdk-16: libc.so fixup failed" >&2; exit 1; fi
...
[ -n "$(echo "${CXXINC}" | grep -o 'c++')" ] || { echo "openjdk-16: cannot find g++'s libstdc++ include dirs" >&2; exit 1; }
...
BOOT=/usr/lib/openjdk-15
# --- P2 unpack (the GitHub tag archive; shipped binaries and jars removed, as Guix does) ---
mkdir src && tar -xzf jdk-16.0.2-ga.tar.gz -C src --strip-components=1
cd src
...
patch -p1 < ../openjdk-16-classlist-reproducibility.patch
# --- source fixes ---
...
bash ./configure --with-boot-jdk="${BOOT}" --disable-option-checking --disable-warnings-as-errors --with-native-debug-symbols=none \
  ...
  || { grep -n -iE 'error|could not|cannot|not found' ../configure.log | tail -8 >&2 || true; echo "openjdk-16: configure failed" >&2; exit 1; }
make JOBS="${JOBS}" all > ../make.log 2>&1 \
  || { grep -nE 'error:|Error [0-9]|\*\*\* \[' ../make.log | grep -v Werror | tail -8 >&2 || true; tail -10 ../make.log >&2; echo "openjdk-16: make failed" >&2; exit 1; }
IMG="$(ls -d build/*/images/jdk | head -1)"
[ -x "${IMG}/bin/java" ] || { echo "openjdk-16: no jdk image" >&2; exit 1; }
...
[ "$OUT" = "OJ:21:16:abab" ] || { echo "openjdk-16: gate printed '$OUT'" >&2; exit 1; }
