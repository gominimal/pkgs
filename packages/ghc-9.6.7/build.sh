...
if grep -rl "${BUILDROOT}" "${DST}/bin" "${SETTINGS}" "${DB}" -q 2>/dev/null; then
  grep -rl "${BUILDROOT}" "${DST}/bin" "${SETTINGS}" "${DB}" >&2; echo "ghc-${VERSION}: build paths survive in the install" >&2; exit 1; fi
